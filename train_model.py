...
def train_xgboost_ranker_v3():
    logging.info("=== PHASE 8.2: TRAINING XGBOOST RANKER (V3 - Pace Topology) ===")
    
    # 1. Load Data
    conn = sqlite3.connect('hk_racing.db')
    logging.info("Loading V3 feature matrix from vault...")
    df = pd.read_sql("SELECT * FROM ml_features", conn)
    conn.close()
    
    # CRITICAL FIX: Strip the scraping duplicates created during the Left Join
    initial_len = len(df)
    df = df.drop_duplicates(subset=['date', 'race_id', 'horse_id'])
    logging.info(f"Deduplication: Removed {initial_len - len(df)} ghost rows.")
    ...
    # NEW: The V3 Feature Set (Now includes Orthogonal Pace Dynamics)
    features = [
        'pre_race_elo', 'days_since_last_run', 'weight_delta', 
        'distance_delta', 'career_wins', 'is_turf', 
        'draw', 'jockey_win_pct', 'trainer_win_pct',
        'shifted_rolling_ESI', 'shifted_rolling_CSI', # Speed Indices
        'race_ESI_pressure', 'pace_advantage'         # Race Context
    ]
    
    for col in features:
        df[col] = pd.to_numeric(df[col], errors='coerce').astype(float)
        
    # We drop NaNs here. First-time starters (who have no pace history) are dropped.
    # We only want the AI learning from established pace dynamics.
    df = df.dropna(subset=features)
    
    # Strict Intra-Race Sorting for Pairwise Logic
    ...
    ranker = xgb.XGBRanker(
        ...
        max_depth=5,
        subsample=0.8,
        colsample_bytree=0.5,  # Force exploration of Pace features
        n_estimators=1000,
        early_stopping_rounds=100
    )
    
    logging.info("Training V3 Pace-Adjusted Model...")
    ranker.fit(
        X_train, y_train,
        group=group_train,
    ...
    # Save the V3 model
    ranker.save_model('v3_hkjc_ranker.json')
    logging.info("\nModel saved to 'v3_hkjc_ranker.json'. Ready for Divergence Backtesting.")

if __name__ == "__main__":
    train_xgboost_ranker_v3()