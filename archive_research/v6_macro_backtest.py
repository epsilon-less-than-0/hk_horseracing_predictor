...
def run_macro_backtest():
    logging.info("=== PHASE 12: THE MACRO STRESS TEST (V6.0) ===")
    
    # 1. Extract Data
    logging.info("Extracting master vault data...")
    ...
    logging.info(f"Macro Splits -> Train (Ranker): {len(train_df):,} | Calib (Meta): {len(calib_df):,} | Test (Execution): {len(test_df):,}")

    if len(test_df) == 0:
        logging.error("Test data is empty. Check your database dates.")
        return

    # 3. Stage 1: Train the Targeter (Ranker)
    logging.info("\nStage 1: Training XGBRanker (2018-2021 data)...")
    ...
    # 4. Stage 2: Train the Calibrator (Logistic Meta-Model)
    logging.info("Stage 2: Training Logistic Calibrator (2022 data)...")
    ...
    brier = brier_score_loss(y_calib, calibrator.predict_proba(X_calib)[:, 1])
    logging.info(f"Calibrator Brier Score: {brier:.4f}")

    # 5. Stage 3: The Macro Execution Desk (2023-2025)
    logging.info(f"\nStage 3: Running Calibrated Kelly Desk on {test_df['date'].dt.year.nunique()} years of unseen data...")
    ...
    for (date, race_id), race_data in races:
        ...
        # FILTER: Block the Consensus Trap
        ...
        if f_star > 0:
            bet_pct = min((f_star * KELLY_FRACTION), MAX_BET_CAP)
    ...
    logging.info("\n=== MACRO STRESS TEST DEBRIEF ===")
    logging.info(f"Time Horizon:       2023 - 2025")
