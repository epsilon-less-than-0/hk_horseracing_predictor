...
class WinProbabilityCalibrator:
    ...
    def transform(self, test_df: pd.DataFrame, score_col: str = 'model_score') -> pd.DataFrame:
        ...
        test_df['calibrated_win_prob'] = self.calibrator.predict_proba(X_test)[:, 1]
        return test_df
...
def main():
    logging.info("=== INITIALIZING PHASE 51: TRUE KINETIC MAX-ALPHA DESK ===")
    ...
    # NEW: Isolate strictly to the Win constraint for Harville mapping
    ...
    logging.info("Fitting Logistic Platt Calibration (P_WIN target)...")
    ...
    BANKROLL = 100000.0
    MIN_TICKET_COST = 10.0 
    KELLY_MULT = 0.10 
    EV_THRESHOLD = 1.05 
    RAKE = 0.23 
    
    logging.info("Executing True Kinetic Harville Sweep...")
    total_action = 0.0
    trios_hit = 0
    races_bet = 0
    max_drawdown = 0.0
    ...
    for (r_date, r_id), race_data in races:
        ...
        # Normalize Public P(Win)
        race_data['inv_odds'] = 1.0 / race_data['public_odds']
        race_data['prob_pub'] = race_data['inv_odds'] / race_data['inv_odds'].sum()
        
        # Normalize Engine P(Win) - Now mathematically sound as we trained on is_win
        race_data['prob_eng'] = race_data['calibrated_win_prob'] / race_data['calibrated_win_prob'].sum()
        
        top_7 = race_data.nlargest(7, 'calibrated_win_prob').copy()
        ...
        if best_combo is not None:
            ...
            if ticket_wager >= MIN_TICKET_COST:
                ...
                if combo_df[combo_df['finish_position'] <= 3.0].shape[0] == 3:
                    trios_hit += 1
                    BANKROLL += (ticket_wager / 10.0) * (best_odds * 10.0)
    ...
    print("\n" + "="*50)
    print("=== FINAL V28 TRUE KINETIC ISOLATION LEDGER ===")
    print("="*50)
