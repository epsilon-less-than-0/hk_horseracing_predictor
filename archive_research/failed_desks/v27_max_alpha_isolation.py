...
def main():
    logging.info("=== INITIALIZING PHASE 50: MAX-ALPHA ISOLATION DESK ===")
    ...
    # 4. Engine Training & Platt Calibration
    ...
    logging.info("Fitting Logistic Platt Calibration...")
    ...
    # --- THE MAX-ALPHA PORTFOLIO ARCHITECTURE ---
    BANKROLL = 100000.0
    ...
    logging.info("Executing Simultaneous Dutching Sweep (Max-Alpha Targeting)...")
    total_action = 0.0
    trios_hit = 0
    races_bet = 0
    max_drawdown = 0.0
    ...
    for (r_date, r_id), race_data in races:
        ...
        # 1. Map & Normalize Public Probability Universe 
        ...
        # 2. Map & Normalize Engine Probability Universe
        # We normalize the Platt calibrated probabilities to sum to 1.0 across the race
        # to ensure Harville denominators do not collapse structurally.
        engine_mass_total = race_data['calibrated_place_prob'].sum()
        race_data['prob_eng'] = race_data['calibrated_place_prob'] / engine_mass_total
        ...
        # 3. Combinatorial Alpha Sweep: Evaluate all 35 Combinations
        for combo in itertools.combinations(horse_ids, 3):
            ...
            if combo_ev >= EV_THRESHOLD:
                ...
                # MAX-ALPHA ISOLATION: We only retain the ticket with the highest Kelly multiplier
                if fractional_kelly > best_kelly:
                    best_kelly = fractional_kelly
                    best_combo = combo
                    best_odds = combo_odds
                    
        # 4. Execution Module (Single Targeted Strike)
        if best_combo is not None:
            ...
            if ticket_wager >= MIN_TICKET_COST:
                ...
                # Track Covariance-Free Drawdown
    ...
    print("\n" + "="*50)
    print("=== FINAL V27 MAX-ALPHA ISOLATION LEDGER ===")
    print("="*50)
