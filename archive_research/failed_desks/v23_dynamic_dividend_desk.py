...
def compute_synthetic_harville_dividend(anchor_odds, legs_odds_list, rake=0.175):
    """
    Computes a synthetic Tierce payout based on the Harville joint probability distribution.
    Takes the average expected dividend across all combinations in our combination wheel.
    """
    p_anchor = 1.0 / anchor_odds
    
    synthetic_payouts = []
    
    # Evaluate all valid permutations of 2nd and 3rd place from our selected legs
    for i in range(len(legs_odds_list)):
        for j in range(len(legs_odds_list)):
            if i == j: continue
            p_leg2 = 1.0 / legs_odds_list[i]
            p_leg3 = 1.0 / legs_odds_list[j]
            
            # Harville formulation for exact ordered outcome (1st, 2nd, 3rd)
            p_joint = p_anchor * (p_leg2 / (1.0 - p_anchor)) * (p_leg3 / (1.0 - p_anchor - p_leg2))
            
            # Bound probability to prevent mathematical infinity splits
            p_joint = max(min(p_joint, 0.99), 0.00001)
            
            # Dynamic payout allocation incorporating track takeout
            dividend_per_dollar = (1.0 / p_joint) * (1.0 - rake)
            synthetic_payouts.append(dividend_per_dollar)
            
    # Return the expected central payout value of our 12-ticket wheel
...
def main():
    logging.info("=== INITIALIZING PHASE 45: DYNAMIC DIVIDEND PRODUCTION DESK ===")
    ...
    KELLY_MULT = 0.10 # Hard 1/10th Kelly
    ...
    logging.info("Executing Live Simulated Portfolio Compounding with Harville Dividends...")
    total_action = 0.0
    tierces_hit = 0
    races_bet = 0
    max_drawdown = 0.0
    ...
    for (r_date, r_id), race_data in races:
        if BANKROLL <= MIN_TICKET_COST:
            logging.error("EMERGENCY STOP: Capital Liquidity Depleted.")
            break
        ...
        # --- SYNTHETIC DIVIDEND COUPLING ---
        legs_odds = legs['public_odds'].tolist()
        # Compute exact expected payout ratio per unit bet for this unique combination
        synthetic_payout_ratio = compute_synthetic_harville_dividend(anchor['public_odds'], legs_odds)
        
        # Convert total payout ratio to base unit decimal odds (per $1.00 wagered)
        # Since our ticket cost spans 12 combinations, decimal odds are scaled by the wheel structure
        decimal_odds_input = synthetic_payout_ratio / TICKET_COMBINATIONS
        
        # Dynamic Kelly calculation using the unique payout density of this specific race setup
        regime_kelly_fraction = calculate_dynamic_kelly(p_regime, decimal_odds_input, KELLY_MULT)
        ...
        if anchor_win and (legs_in_top_3 >= 2):
            tierces_hit += 1
            # Realized payout tracks the actual unique synthetic dividend generated by the combo
    ...
    print("\n" + "="*50)
    print("=== FINAL DYNAMIC PRODUCTION TIERCE LEDGER ===")
    print("="*50)
