...
def run_value_roi_backtest():
    logging.info("=== PHASE 16: THE VALUE EXOTIC DESK (V8.2) ===")
    logging.info("Strategy: 3-Horse Box")
    logging.info("Filter 2: Minimum Expected Dividend > $45")
    logging.info("Sizing: Value-Proportional (Dynamic Bankroll Allocation)")
    ...
    for (date, race_id), race_data in races:
        ...
        for horse_A, horse_B in combos:
            ...
            expected_payout = 10.0 * actual_payout_multiplier # Theoretical payout for a standard $10 ticket
            
            # FILTER 2: Refuse to bet if the payout is too small (The Consensus Penalty)
            if expected_payout < MINIMUM_PAYOUT_THRESHOLD:
                continue
                
            # DYNAMIC SIZING: Scale bet based on expected payout magnitude
            # Base risk is 1% of bankroll. Scales up with payout. Cap at 3.5% per combo.
            bet_fraction = min(0.01 * (expected_payout / MINIMUM_PAYOUT_THRESHOLD), 0.035)
            wager = bankroll * bet_fraction
            
            race_action += wager
            combinations_bet += 1
            bet_placed_in_race = True
            
            # Did this specific combination win?
            if (horse_A['plc_num'] in [1.0, 2.0]) and (horse_B['plc_num'] in [1.0, 2.0]) and (horse_A['plc_num'] != horse_B['plc_num']):
                quinellas_hit += 1
                # We won! Payout is our wager * the theoretical multiplier
                race_payout += (wager * actual_payout_multiplier)
        
        if bet_placed_in_race:
            races_played += 1
    ...
    logging.info("=== VALUE DESK DEBRIEF (2023-2025) ===")
    logging.info(f"Total Races Played:  {races_played}")
    logging.info(f"Combinations Bet:    {combinations_bet}")
    logging.info(f"Winning Tickets:     {quinellas_hit}")
